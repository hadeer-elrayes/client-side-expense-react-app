import {AUTH_ATTEMPTING , AUTH_FAILD ,AUTH_SUCCESS ,USER_LOGEDOUT , FETCHED_PROFILE} from './types';
...
export const logIn = request_data => {
     return async dispatch => {
       dispatch({type:AUTH_ATTEMPTING})
         try{
           const {data} = await axios.post("http://localhost:3000/users/authentication" , request_data);
...
export const fetchProfile = (token) =>{
  return async dispatch =>{
    ...
    const {data :{user}} = await axios.get("http://localhost:5000/users/profile",token)
...
const success = (token)=>{
  localStorage.setItem('expense-app-token',token)
